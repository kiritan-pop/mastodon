...
export function submitCompose(visOrSuccessCallback = null, successCallback = null) {
  ...
  if (typeof visOrSuccessCallback === 'function') {
    callback = visOrSuccessCallback;
  } else if (visOrSuccessCallback !== null && typeof visOrSuccessCallback !== 'undefined') {
    visibilityOverride = visOrSuccessCallback;
  }
  ...
  return function (dispatch, getState) {
    ...
    const visibility = visibilityOverride ?? getState().getIn(['compose', 'privacy']);
    api().request({
...
export function fetchComposeSuggestions(token) {
  return (dispatch, getState) => {
    ...
    case ':':
      if (token[1] === '@') {
        fetchComposeSuggestionsAccounts(dispatch, getState, token.slice(1, token.length + 1));
      } else {
...
export function selectComposeSuggestion(position, token, suggestion, path) {
  return (dispatch, getState) => {
    ...
    if (suggestion.type === 'emoji') {
      ...
      const acct = getState().getIn(['accounts', suggestion.id, 'acct']);
      if (token[0] === ':') {
        completion    = `@${acct}:`;
