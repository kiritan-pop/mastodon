...
export function normalizeAccount(account) {
  ...
  const emojiMap = makeEmojiMap(account.all_emojis);
...
export function normalizeStatus(status, normalOldStatus) {
  ...
  if (normalOldStatus && normalOldStatus.get('content') === normalStatus.content && normalOldStatus.get('spoiler_text') === normalStatus.spoiler_text) {
    ...
    const searchContent = ([spoilerText, status.content].concat((status.poll && status.poll.options) ? status.poll.options.map(option => option.title) : [])).concat(status.media_attachments.map(att => att.description)).join('\n\n').replace(/<br\s*\/?>/g, '\n').replace(/<\/p><p>/g, '\n\n');
    const emojiMap      = makeEmojiMap(normalStatus.all_emojis);
...
export function normalizeStatusTranslation(translation, status) {
  const emojiMap = makeEmojiMap(status.get('all_emojis').toJS());
...
export function normalizeAnnouncement(announcement) {
  ...
  const emojiMap = makeEmojiMap(normalAnnouncement.all_emojis);
